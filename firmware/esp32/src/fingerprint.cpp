...
bool FingerprintManager::deleteID(int id) {
    ...
    return true;
#else
    Serial.printf("[FP] ID %d silme baslatildi.\n", id);

    bool existedInNvs = false;
    fpStore.begin("fingerprints", false);
    String key = "fp_" + String(id);
    String existKey = "ex_" + String(id);
    existedInNvs = fpStore.getBool(existKey.c_str(), false);

    if (existedInNvs) {
        int count = fpStore.getInt("count", 0);
        fpStore.remove(key.c_str());
        fpStore.remove(existKey.c_str());
        fpStore.putInt("count", max(0, count - 1));
        Serial.printf("[FP] NVS kaydi silindi. id=%d onceki_count=%d yeni_count=%d\n",
                      id, count, max(0, count - 1));
    } else {
        Serial.printf("[FP] NVS kaydi bulunamadi. id=%d delete idempotent kabul ediliyor.\n", id);
        fpStore.remove(key.c_str());
        fpStore.remove(existKey.c_str());
    }
    fpStore.end();

    if (_initialized && _finger) {
        int result = _finger->deleteModel(id);
        if (result == FINGERPRINT_OK) {
            Serial.printf("[FP] Sensor flash kaydi silindi. id=%d\n", id);
        } else {
            Serial.println("[FP] Sensor flash silme best-effort basarisiz: " + errorToString(result));
        }
    } else {
        Serial.println("[FP] Sensor hazir degil; NVS silindi, sensor flash silme atlandi.");
    }

    Serial.printf("[FP] ID %d silme tamamlandi.\n", id);
    return true;
#endif
}
...
// ============================================================================
// hasFinger() - Hizli Parmak Algilama
// ============================================================================

/**
 * @brief Sensorde parmak olup olmadigini hafif bir getImage() ile kontrol eder.
 */
bool FingerprintManager::hasFinger() {
#ifdef MOCK_MODE
    return false;
#else
    if (!_initialized || !_finger) {
    ...
    if (p != FINGERPRINT_NOFINGER) {
        Serial.println("[FP] Parmak kontrol hatasi: " + errorToString(p));
    }
